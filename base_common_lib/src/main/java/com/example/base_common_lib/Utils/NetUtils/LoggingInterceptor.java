...
public class LoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        LogUtils.workResquset(String.format("接收响应: [%s] %n返回json:【%s】 %.1fms %n%s",
    ...
    private void postAndGet(String method, Request request, Chain chain) {
        if ("POST".equals(method)) {
            ...
            if (request.body() instanceof FormBody) {
                ...
                LogUtils.workPost(String.format("发送请求 %s on %s %n%s %nRequestParams:{%s}",
...
//
            LogUtils.workPost( String.format("发送请求 %s on %s%n%s",
